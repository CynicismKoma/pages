---
title: "Flume 整合 Kafka_kafka和flume的项目"
pubDatetime: 2022-07-04T14:34:35
description: "一、背景 先说一下，为什么要使用 Flume + Kafka？ 以实时流处理项目为例，由于采集的数据量可能存在峰值和峰谷，假设是一个电商项目，那么峰值通常出现在秒杀时，这时如果直接将 Flume 聚合后的数据输入到 Storm 等分布式计算框架中，可能就会超过集群的处理能力，这时采用 Kafka 就可以起到削峰的作用。Kafka 天生为大数据场景而设计，具有高吞吐的特性，能很好地抗住峰值数据的冲击"
tags: []
draft: false
---
### 一、背景
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b44354e904acac80f689a8814027171a.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/85ecdd1a8583d7dd13cd20a2cd180cad.png#pic_center)

可以看到 `flume-kafka` 主题的消费端已经收到了对应的消息：

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/8f697ff8ad5185c123a9af2eed2b251d.png#pic_center)