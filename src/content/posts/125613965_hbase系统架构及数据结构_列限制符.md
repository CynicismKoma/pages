---
title: "Hbase系统架构及数据结构_列限制符"
pubDatetime: 2022-07-05T10:04:06
description: "一、基本概念 一个典型的 Hbase Table 表如下： 1.1 Row Key (行键) Row Key 是用来检索记录的主键。想要访问 HBase Table 中的数据，只有以下三种方式： 通过指定的 Row Key 进行访问； 通过 Row Key 的 range 进行访问，即访问指定范围内的行； 进行全表扫描。 Row Key 可以是任意字符串，存储时数据按照 Row Key 的字典序进"
tags: []
draft: false
---
### 一、基本概念
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/6b21695666dee8583544a2103abfda33.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3564f0ec653060c6c74a174d4590f919.png#pic_center)

每个表一开始只有一个 `Region`，随着数据不断增加，`Region` 会不断增大，当增大到一个阀值的时候，`Region` 就会等分为两个新的 `Region`。当 Table 中的行不断增多，就会有越来越多的 `Region`。

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/58bf550872787a63ecc4e17533a4bf0d.png#pic_center)

`Region` 是 HBase 中**分布式存储和负载均衡的最小单元** 。这意味着不同的 `Region` 可以分布在不同的 `Region Server` 上。但一个 `Region` 是不会拆分到多个 Server 上的。

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/06269bfd0678acae96fa5060364dbd07.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/8c12e8a4deca9fe6da35d58118355238.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e3dcf806319bd5975eed44b6f506e2ed.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/01103f4d6bd8046df2a67dfcfbca12ef.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/739305eaf655c2caef3d1a043c3a25b7.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/716c4dabddc42e5355f6ec64f30d94c9.png#pic_center)
