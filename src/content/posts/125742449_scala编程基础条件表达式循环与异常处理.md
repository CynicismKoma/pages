---
title: "Scala编程基础：条件表达式、循环与异常处理"
pubDatetime: 2022-07-12T14:28:37
description: "一、条件表达式if Scala 中的 if/else 语法结构与 Java 中的一样，唯一不同的是，Scala 中的 if 表达式是有返回值的。 object ScalaApp extends App { val x = 'scala' val result = if ( x . length == 5 ) 'true' else 'false' print ( result ) } AI写代码 "
tags: []
draft: false
---
### 一、条件表达式if
