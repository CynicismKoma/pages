---
title: "Scala简介及开发环境配置_scala2.12.11版本对应什么版本scala-maven-plugin"
pubDatetime: 2022-07-12T14:32:54
description: "一、Scala简介 1.1 概念 Scala 全称为 Scalable Language，即“可伸缩的语言”，之所以这样命名，是因为它的设计目标是希望伴随着用户的需求一起成长。Scala 是一门综合了 面向对象 和 函数式编程概念 的 静态类型 的编程语言，它运行在标准的 Java 平台上，可以与所有的 Java 类库无缝协作。 1.2 特点 1. Scala是面向对象的 Scala 是一种面向对"
tags: []
draft: false
---
### 一、Scala简介
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/96d267d2e1dae3a86aa7992b7ac9057d.png#pic_center)

#### 2.3 创建Scala项目

在 IDEA 中依次点击 **File** => **New** => **Project** 选项卡，然后选择创建 `Scala—IDEA` 工程：

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e9736556308c5ae1e75b14fab33ed87d.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f61d66b679d9668d66ae24548ad49502.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e0d11d2c0acfef1d285cd0dde817a74f.png#pic_center)

由于安装时已经自动配置好环境变量，所以 IDEA 会自动选择对应版本的 SDK。

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2ada6701c352c46290d095e50cb00f11.png#pic_center)

#### 2.5 创建Hello World

在工程 `src` 目录上右击 **New** => **Scala class** 创建 `Hello.scala`。输入代码如下，完成后点击运行按钮，成功运行则代表搭建成功。

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/743a91a2b9cf578b1800794fdbf90e24.png#pic_center)

#### 2.6 切换Scala版本

在日常的开发中，由于对应软件（如 Spark）的版本切换，可能导致需要切换 Scala 的版本，则可以在 `Project Structures` 中的 `Global Libraries` 选项卡中进行切换。

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9814aca0c07b32ad4f5aa0ceadd73aa1.png#pic_center)

#### 2.7 使用scala命令行

采用 `msi` 方式安装，程序会自动配置好环境变量。此时可以直接使用命令行工具：

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b7bf7d1854e1c7a9c057cee5b02bb373.png#pic_center)