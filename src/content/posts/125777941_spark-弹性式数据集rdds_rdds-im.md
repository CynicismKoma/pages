---
title: "Spark 弹性式数据集RDDs_rdds- .im,"
pubDatetime: 2022-07-14T09:51:57
description: "一、RDD简介 RDD 全称为 Resilient Distributed Datasets，是 Spark 最基本的数据抽象，它是只读的、分区记录的集合，支持并行操作，可以由外部数据集或其他 RDD 转换而来，它具有以下特性： 一个 RDD 由一个或者多个分区（Partitions）组成。对于 RDD 来说，每个分区会被一个计算任务所处理，用户可以在创建 RDD 时指定其分区个数，如果没有指定，"
tags: []
draft: false
---
### 一、RDD简介
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0b6d52021adb6d5c5a04a15f4828d100.png#pic_center)
...
RDD 支持两种类型的操作： _transformations_ （转换，从现有数据集创建新数据集）和 _actions_ （在数据集上运行计算后将值返回到驱动程序）。RDD 中的所有转换操作都是惰性的，它们只是记住这些转换操作，但不会立即执行，只有遇到 _action_ 操作后才会真正的进行计算，这类似于函数式编程中的惰性求值。
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b7db834b2109370dc5d069574acb01d5.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/ca2851bf0151f7dfcca05373260cc898.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e32618da0256aa30cb00d92ceda80bf7.png#pic_center)