---
title: "Spark 累加器与广播变量_spark的累积器可以被用于广播变量一起使用嘛"
pubDatetime: 2022-07-18T10:50:37
description: "一、简介 在 Spark 中，提供了两种类型的共享变量：累加器 (accumulator) 与广播变量 (broadcast variable)： 累加器 ：用来对信息进行聚合，主要用于累计计数等场景； 广播变量 ：主要用于在节点间高效分发大对象。 二、累加器 这里先看一个具体的场景，对于正常的累计求和，如果在集群模式中使用下面的代码进行计算，会发现执行结果并非预期： var counter = "
tags: []
draft: false
---
### 一、简介
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/fd6a102054761bae92abd671dad02cc3.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/5b80445cc38dbf6e3f58aa8b18e8bfb2.png#pic_center)

#### 2.2 使用累加器

`SparkContext` 中定义了所有创建累加器的方法，需要注意的是：被中横线划掉的累加器方法在 Spark 2.0.0 之后被标识为废弃。

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2da20e1d3b4c3b5267032b71aef9dde0.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/ef9530d7818192562319d765dbeed5e5.png#pic_center)
